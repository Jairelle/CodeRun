...
*   **Bidirectional Inter-Process Signaling**: Employs a local signal file (`signal.txt`) to coordinate return-to-home commands from the Unity application, automatically ending the sub-process and reloading the main dashboard menu.
...
└── WinFormsApp1.slnx       # Solution configuration file
```
---
## ⚙️ How it Works
...
2.  Open the solution file [WinFormsApp1.slnx](file:///c:/Users/oreto/source/repos/WinFormsApp1/WinFormsApp1.slnx) in Visual Studio.
3.  Ensure the executable files exist in their respective directories (e.g. `GameSceneEasyMode\My project (2).exe`).
4.  Build and run the project `CodeRun`.
